...
export function ChequePaymentPanel({
  branch = 'Nabunturan',
  onClose,
}: ChequePaymentPanelProps) {
  const { user } = useAuth()
  const activeBranch = branch ?? 'Nabunturan'
  const kind = 'cheque_payment' as const
  ...
  const [draftChequeNo, setDraftChequeNo] = useState('')
  const [draftAmount, setDraftAmount] = useState('')
  const [draftDueDate, setDraftDueDate] = useState(toIsoDateInput())
  ...
  function clearDraft() {
    setDraftChequeNo('')
    setDraftAmount('')
    setDraftDueDate(toIsoDateInput())
  }
  ...
  function handleAddLine() {
    const chequeNo = draftChequeNo.trim()
    const amount = Number(draftAmount)
    const dueDate = draftDueDate
    if (!chequeNo) {
      setError('Enter a cheque number.')
      return
    }
  ...
  async function handleSave() {
    if (lines.length === 0) {
      setError('Add at least one cheque before saving.')
      return
    }
    setSaving(true)
    setError(null)
    const result = await saveMtsCollection({
      ...
      items: lines.map((line) => ({
        qty: 1,
        unit: '',
        description: line.chequeNo,
        price: line.amount,
        totalAmount: line.amount,
        remarks: line.dueDate,
      })),
    ...
    if (result.error || !result.data) {
      setError(result.error ?? 'Failed to save cheque payment.')
      return
    }
    ...
    setSavePopup({
      ...
      detail: wasUpdate
        ? `CHEQUE PAYMENT was updated for ${activeBranch}.`
        : `CHEQUE PAYMENT was saved for ${activeBranch}.`,
    })
    resetForm()
  }
  ...
  async function loadRecord(id: string) {
    ...
    setLines(
      detail.data.items.map((item) => ({
        id: item.id,
        chequeNo: item.description,
        amount: Number(item.total_amount) || Number(item.price) || 0,
        dueDate: item.remarks?.trim() || detail.data!.collection.collection_date,
      })),
    )
    setSearchOpen(false)
    setError(null)
  ...
  return (
    <section className="mts-collections mts-collections--cheque" aria-label="CHEQUE PAYMENT">
      ...
      <div className="mts-collections__body">
        ...
        <div className="mts-collections__table-wrap">
          <table className="mts-collections__table">
            <thead>
              <tr>
                <th className="is-cheque-no">CHEQUE NO.</th>
                <th className="is-amount">AMOUNT</th>
                <th className="is-due-date">DUE DATE</th>
            ...
            <tbody>
              {lines.length === 0 ? (
                ...
                lines.map((line) => (
                  <tr key={line.id}>
                    <td className="is-cheque-no">{line.chequeNo}</td>
                    <td className="is-amount">{money(line.amount)}</td>
                    <td className="is-due-date">{formatDisplayDate(line.dueDate)}</td>
        ...
        <div
          className="mts-collections__entry mts-collections__entry--cheque"
        ...
        >
          <label>
            ...
            <input
              type="text"
              value={draftChequeNo}
              onChange={(event) => setDraftChequeNo(event.target.value)}
            />
          </label>
          <label>
            <span>AMOUNT</span>
      ...
      {searchOpen ? (
        <div className="modal-backdrop" onClick={() => setSearchOpen(false)} role="presentation">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="cheque-payment-search-title"
            onClick={(event) => event.stopPropagation()}
          >
            <header className="mts-collections-search__head">
              <h2 id="cheque-payment-search-title">Search Cheque Payment</h2>
