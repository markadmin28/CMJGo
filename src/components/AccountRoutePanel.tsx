...
function CashIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <rect x="3" y="6" width="18" height="12" rx="2" stroke="#16a34a" strokeWidth="1.8" />
      <circle cx="12" cy="12" r="2.5" stroke="#16a34a" strokeWidth="1.8" />
      <path d="M6 10v4M18 10v4" stroke="#16a34a" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  )
}
...
export function AccountRoutePanel({
  branch = 'Nabunturan',
  onClose,
}: AccountRoutePanelProps) {
  const { user } = useAuth()
  const activeBranch = branch ?? 'Nabunturan'
  const kind = 'account_route' as const
  ...
  const balance = totalAmount - cashAmount
  ...
  useEffect(() => {
    function onKeyDown(event: globalThis.KeyboardEvent) {
      if (event.key === 'Escape' && !searchOpen && !cashOpen) onClose?.()
    }
    window.addEventListener('keydown', onKeyDown)
    return () => window.removeEventListener('keydown', onKeyDown)
  }, [onClose, searchOpen, cashOpen])

  useEffect(() => {
    if (cashOpen) {
      setCashDraft(cashAmount ? String(cashAmount) : '')
  ...
  async function handleSave() {
    ...
    if (result.error || !result.data) {
      setError(result.error ?? 'Failed to save account route.')
      return
    }
    ...
    setSavePopup({
      ...
      detail: wasUpdate
        ? `ACCOUNT ROUTE was updated for ${activeBranch}.`
        : `ACCOUNT ROUTE was saved for ${activeBranch}.`,
    })
    resetForm()
  }
  ...
  async function loadRecord(id: string) {
    ...
    setPlateNo(detail.data.collection.plate_no ?? '')
    setFromName(detail.data.collection.from_name)
    setCashAmount(Number(detail.data.collection.cash_amount) || 0)
    setLines(
  ...
  return (
    <section className="mts-collections mts-collections--account-route" aria-label="ACCOUNT ROUTE">
      ...
      <div className="mts-collections__body">
        <div className="mts-collections__fields mts-collections__fields--route">
        ...
        <div className="mts-collections__table-wrap">
          ...
          <div className="mts-collections__summary">
            <div>
              <span>TOTAL :</span>
            ...
            <div className="mts-collections__summary-bal">
              <span>BAL :</span>
        ...
        <div className="mts-collections__entry mts-collections__entry--route" onKeyDown={handleEntryKeyDown}>
          <label>
            <span>QTY</span>
            <input
              type="number"
          ...
          <div className="mts-collections__entry-actions">
            ...
            <button
              type="button"
              className="mts-collections__cash"
      ...
      {cashOpen ? (
        <div className="modal-backdrop" onClick={() => setCashOpen(false)} role="presentation">
          <div
            className="mts-collections-search mts-collections-cash-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="account-route-cash-title"
            onClick={(event) => event.stopPropagation()}
          >
            <header className="mts-collections-search__head">
              <h2 id="account-route-cash-title">Enter Cash</h2>
              <button type="button" onClick={() => setCashOpen(false)} aria-label="Close cash">
                <CloseIcon />
              </button>
            </header>
            <label className="mts-collections-cash-modal__field">
            ...
            <div className="mts-collections-cash-modal__actions">
              <button type="button" onClick={() => setCashOpen(false)}>
                Cancel
              </button>
              <button type="button" onClick={applyCash}>
                Apply
              </button>
            </div>
          </div>
      ...
      {searchOpen ? (
        <div className="modal-backdrop" onClick={() => setSearchOpen(false)} role="presentation">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="account-route-search-title"
            onClick={(event) => event.stopPropagation()}
          >
            <header className="mts-collections-search__head">
              <h2 id="account-route-search-title">Search Account Route</h2>
