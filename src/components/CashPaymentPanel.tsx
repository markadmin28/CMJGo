...
type CashPaymentPanelProps = {
  ...
  kind?: Extract<MtsLedgerKind, 'cash_payment'>
  title?: string
  onClose?: () => void
}
...
export function CashPaymentPanel({
  ...
  const [draftCash, setDraftCash] = useState('Cash')
  ...
  function handleAddLine() {
    const amount = Number(draftAmount)
    const cash = draftCash.trim() || 'Cash'
  ...
  async function handleSave() {
    ...
    if (result.error || !result.data) {
      setError(result.error ?? 'Failed to save cash payment.')
      return
    }
    ...
    setSavePopup({
      ...
      detail: wasUpdate
        ? `${title} was updated for ${activeBranch}.`
        : `${title} was saved for ${activeBranch}.`,
    })
    resetForm()
  }
  ...
  return (
    <section className="mts-collections mts-collections--cash" aria-label={title}>
      ...
      <div className="mts-collections__body">
          ...
          <table className="mts-collections__table">
            ...
            <tbody>
              {lines.length === 0 ? (
                ...
                lines.map((line) => (
                  <tr key={line.id}>
                    <td className="is-cash">{line.cash}</td>
        ...
        <div
          className="mts-collections__entry mts-collections__entry--cash"
        ...
        >
          <label>
            ...
            <input
              type="text"
              value={draftCash}
              onChange={(event) => setDraftCash(event.target.value)}
            />
          </label>
      ...
      {searchOpen ? (
        <div className="modal-backdrop" onClick={() => setSearchOpen(false)} role="presentation">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="cash-payment-search-title"
            onClick={(event) => event.stopPropagation()}
          >
            <header className="mts-collections-search__head">
              <h2 id="cash-payment-search-title">Search {title}</h2>
