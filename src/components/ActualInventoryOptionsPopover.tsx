...
export type ActualInventoryCompany = 'Pepsi' | 'SMC' | 'Magnolia'

type ActualInventoryOptionsPopoverProps = {
  open: boolean
  top: number
  left: number
  onClose: () => void
  onSelect: (company: ActualInventoryCompany) => void
}

const COMPANIES: ActualInventoryCompany[] = ['Pepsi', 'SMC', 'Magnolia']

function optionClass(company: ActualInventoryCompany) {
  if (company === 'Pepsi') return 'pepsi'
  if (company === 'SMC') return 'smc'
  return 'magnolia'
}

export function ActualInventoryOptionsPopover({
  open,
  top,
  left,
  onClose,
  onSelect,
}: ActualInventoryOptionsPopoverProps) {
  ...
  return (
    <div
      ...
      role="menu"
      aria-label="Actual Inventory options"
    >
