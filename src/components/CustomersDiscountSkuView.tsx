...
import { listCustomerDiscounts, saveCustomerDiscounts } from '../lib/customerDiscount'
import './CatalogPanel.css'
import './FthDiscountPanel.css'

type CustomersDiscountSkuViewProps = {
  groupId: string
  branch: UserBranch
}

export function CustomersDiscountSkuView({ groupId, branch }: CustomersDiscountSkuViewProps) {
  ...
  async function loadCatalog() {
    const result = await listCatalogTree(branch, { forTransactions: true })
  ...
  async function loadDiscounts() {
    const result = await listCustomerDiscounts(groupId)
  ...
  useEffect(() => {
    void loadAll()
    // eslint-disable-next-line react-hooks/exhaustive-deps -- reload when customer/branch changes
  }, [groupId, branch])
  ...
  async function handleSave(event: FormEvent) {
    ...
    const result = await saveCustomerDiscounts(
      groupId,
  ...
  if (skuMissing) {
    return (
      <p className="catalog-empty">
        ...
        Register products in Items price first, then return here to set discounts.
      </p>
    )
  }
  ...
  if (discountMissing) {
    return (
      <p className="catalog-empty">
        <span className="catalog-empty-title">Customers discount table missing</span>
        Run the Customers Discount SQL in the setup card above, then refresh.
      </p>
    )
  }
  ...
  if (tree.length === 0) {
    return (
      <p className="catalog-empty">
        ...
        Add categories and products in Items price first.
      </p>
    )
  }
    ...
    <form className="fth-sku-discount" onSubmit={(event) => void handleSave(event)}>
                    ...
                    {subcategory.products.map((product) => (
                      <div key={product.id} className="product-chip fth-discount-chip">
                        ...
                        <input
                          ...
                          placeholder="0.00"
                          aria-label={`Customer discount for ${product.name}`}
                        />
                      </div>
                    ))}
                  </div>
      ...
      <div className="fth-save-bar">
        <button type="submit" className="btn-mini" disabled={submitting}>
          {submitting ? 'Saving…' : 'Save customer discounts'}
        </button>
        {saved ? <span className="fth-save-note">Saved for this customer.</span> : null}
      </div>
    </form>
  )
}
